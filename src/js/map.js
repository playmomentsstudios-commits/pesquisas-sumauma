...
(function () {
  ...
  function popupHtml(r) {
    const parts = [];
    parts.push(`<div>`);
    parts.push(`<div class="popup-title">${esc(r.nome || "")}</div>`);

    const meta = [];
    if (r.territorio) meta.push(esc(r.territorio));
    meta.push(`${esc(r.cidade || "")}/${esc(r.estado || "")}`);
    parts.push(`<div class="popup-meta">${meta.join(" • ")}</div>`);

    if (r.categoria) {
      parts.push(`<div class="popup-cat">${esc(r.categoria)}</div>`);
    }

    const icons = contactIconsHtml(r);
  ...
  function contactIconsHtml(r) {
    const items = [];

    const add = (href, label, svg) => {
      if (!href) return;
      items.push(
        `<a class="contact-icon" href="${esc(href)}" target="_blank" rel="noopener noreferrer" aria-label="${esc(label)}" title="${esc(label)}">
          ${svg}
        </a>`
      );
    };
    ...
    const addTel = (value) => {
      ...
      const href = `tel:${v}`;
      items.push(
        `<a class="contact-icon" href="${esc(href)}" aria-label="Telefone" title="Telefone">
          ${ICON_PHONE}
        </a>`
      );
    };
    ...
    const addMail = (value) => {
      ...
      const href = `mailto:${v}`;
      items.push(
        `<a class="contact-icon" href="${esc(href)}" aria-label="Email" title="Email">
          ${ICON_MAIL}
        </a>`
      );
    };

    add(normalizeUrl(r.site), "Site", ICON_GLOBE);
    add(normalizeHandleUrl(r.instagram, "https://instagram.com/"), "Instagram", ICON_INSTAGRAM);
    add(normalizeFacebook(r.facebook), "Facebook", ICON_FACEBOOK);
    add(normalizeWhatsApp(r.whatsapp), "WhatsApp", ICON_WHATSAPP);
    addMail(r.email);
    add(normalizeYoutube(r.youtube), "YouTube", ICON_YOUTUBE);
    add(normalizeLinkedin(r.linkedin), "LinkedIn", ICON_LINKEDIN);
    add(normalizeHandleUrl(r.tiktok, "https://www.tiktok.com/@"), "TikTok", ICON_TIKTOK);
    addTel(r.telefone);
  ...
  function normalizeLinkedin(v) {
    ...
    if (!s) return "";
    if (/^https?:\/\//i.test(s)) return s;
    if (s.startsWith("in/") || s.startsWith("company/")) {
