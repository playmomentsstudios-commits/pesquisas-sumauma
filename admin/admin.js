...
const handlers = {
  ...
  async handleLogin(e) {
    ...
    const emailInput = form.querySelector('[name="email"]') || document.getElementById('emailInput');
    const passwordInput = form.querySelector('[name="password"]') || document.getElementById('passwordInput');
    ...
    try {
      if (!state.supabase) {
        const initialized = await api.init();
        if (!initialized) throw new Error('Não foi possível conectar ao autenticador.');
      }
      ...
      if (loginHelp) {
        ...
        loginHelp.textContent = 'Confira o e-mail, a senha e a configuração do Supabase.';
      }
    } finally {
  ...
  async handleSaveResearch(e) {
    ...
    try {
      ...
      // Recarrega lista e busca o registro completo para manter o editor consistente
      await this.loadResearches();
      if (result?.id) {
        const fullResearch = await api.researches.getById(result.id);
        state.currentResearch = fullResearch;
        ui.fillResearchForm(fullResearch);
        await this.loadPoints(fullResearch.id);
      }
  ...
  async handleFileSelect(zone, file) {
    ...
    const oldPreviewUrl = zone.dataset.tempUrl;
  ...
  async handleSaveBanner() {
    ...
    try {
      ...
      state.currentResearch = { ...state.currentResearch, banner_url: finalUrl };
      const bannerUrlInput = document.getElementById('bannerUrl');
