...
function fillForm(pesquisa, resumoKV = null){
  ...
  updateReportPreview(pesquisa.relatorio_pdf_url || cfg.relatorioPdf || "");
...
async function savePesquisa(section = "all"){
  ...
  hideAlert();
  setSaveDebug(null);
  const sectionLabel = section === "config" ? "configuração" : section === "resumo" ? "conteúdo da pesquisa" : section === "relatorio" ? "relatório" : section === "ficha" ? "ficha técnica" : "pesquisa";
...
function updateReportPreview(url){
  const frame = document.getElementById("relatorio-preview-frame");
  const empty = document.getElementById("relatorio-preview-empty");
...
window.addEventListener("input", (event) => {
  ...
  if (event.target?.name === "relatorioUrl") {
    ...
    updateReportPreview(event.target.value);
  }
