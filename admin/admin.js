...
const els = {
  ...
  requestAccessBtn: document.getElementById("btn-request-access"),
  ...
  loginHeroBg: document.getElementById("loginHeroBg"),
  loginSiteLogo: document.getElementById("loginSiteLogo"),
  panelHeroBg: document.getElementById("panelHeroBg"),
  panelSiteLogo: document.getElementById("panelSiteLogo"),
  navSiteLogo: document.getElementById("navSiteLogo"),
  relatorioPreview: document.getElementById("relatorio-preview"),
  relatorioPreviewEmpty: document.getElementById("relatorio-preview-empty"),
  mapPreviewSurface: document.getElementById("map-preview-surface"),
  mapPreviewTitle: document.getElementById("map-preview-title"),
  mapPreviewMeta: document.getElementById("map-preview-meta"),
  mapPreviewEmpty: document.getElementById("map-preview-empty"),
  researchSearch: document.getElementById("researchSearch"),
  researchFilterStatus: document.getElementById("researchFilterStatus"),
  previewPublicBtn: document.getElementById("btn-preview-public")
};
...
async function getSiteConfigValue(key){
  try {
    const { data, error } = await state.supabase.from("site_config").select("value").eq("key", key).limit(1).maybeSingle();
    if (error) return "";
    return data?.value || "";
  } catch {
    return "";
...
function applyBrandAsset(imgEl, url){
  if (!imgEl) return;
  if (url) {
    imgEl.src = url;
...
function applyBackground(el, url){
  if (!el) return;
  el.style.backgroundImage = url ? `url("${url}")` : "linear-gradient(135deg, #111827, #1f2937)";
}

async function loadBrandAssets(){
  if (!state.supabase) return;
  const [logoUrl, bannerUrl] = await Promise.all([
    getSiteConfigValue("site_logo_url"),
    getSiteConfigValue("home_banner_url")
  ]);
  applyBrandAsset(els.loginSiteLogo, logoUrl);
  applyBrandAsset(els.panelSiteLogo, logoUrl);
  applyBrandAsset(els.navSiteLogo, logoUrl);
  applyBackground(els.loginHeroBg, bannerUrl);
  applyBackground(els.panelHeroBg, bannerUrl);
}

function updatePdfPreview(url){
  const clean = String(url || "").trim();
  if (els.relatorioPreview) els.relatorioPreview.src = clean || "about:blank";
  if (els.relatorioPreview) els.relatorioPreview.style.display = clean ? "block" : "none";
  if (els.relatorioPreviewEmpty) els.relatorioPreviewEmpty.style.display = clean ? "none" : "block";
}

function updateMapPreview(slug){
  const clean = String(slug || "").trim();
  const surface = els.mapPreviewSurface;
  const empty = els.mapPreviewEmpty;
  if (!surface) return;

  if (!clean) {
    surface.innerHTML = "";
    if (els.mapPreviewTitle) els.mapPreviewTitle.textContent = "Mapa da pesquisa";
    if (els.mapPreviewMeta) els.mapPreviewMeta.textContent = "Selecione uma pesquisa para carregar a prévia.";
    if (empty) empty.style.display = "block";
    return;
  }

  const points = Array.isArray(state.pontos) ? state.pontos.slice(0, 8) : [];
  const palette = ["", "is-featured", "", "", "is-featured", "", ""];
  const fallbackPositions = [
    [18, 24], [34, 56], [50, 38], [63, 68], [77, 28], [24, 74], [84, 58], [58, 18]
  ];

  surface.innerHTML = "";
  points.forEach((ponto, index) => {
    const lat = Number(ponto?.lat);
    const lng = Number(ponto?.lng);
    const hasCoord = Number.isFinite(lat) && Number.isFinite(lng);
    const top = hasCoord ? Math.max(10, Math.min(82, 50 - ((lat + 15) * 0.9))) : fallbackPositions[index % fallbackPositions.length][0];
    const left = hasCoord ? Math.max(8, Math.min(88, 50 + ((lng + 55) * 0.6))) : fallbackPositions[index % fallbackPositions.length][1];
    ...
    label.textContent = ponto?.nome || `Ponto ${index + 1}`;
    node.appendChild(label);
    surface.appendChild(node);
  });

  if (els.mapPreviewTitle) els.mapPreviewTitle.textContent = `Mapa • ${clean}`;
  if (els.mapPreviewMeta) {
    const total = Array.isArray(state.pontos) ? state.pontos.length : 0;
    els.mapPreviewMeta.textContent = total ? `${total} ponto${total > 1 ? "s" : ""} carregado${total > 1 ? "s" : ""} nesta pesquisa.` : "Prévia pronta para receber pontos do CSV ou cadastro manual.";
  }
...
async function init(){
  ...
  els.requestAccessBtn?.addEventListener("click", handleRequestAccess);
  ...
  els.researchSearch?.addEventListener("input", renderFilteredResearchList);
  els.researchFilterStatus?.addEventListener("change", renderFilteredResearchList);
  els.previewPublicBtn?.addEventListener("click", handlePreviewPublicPesquisa);
}
...
function setCurrentResearchId(id){
  ...
  if (!currentResearchId && state.activeEditorTab) {
    try { localStorage.setItem(getTabStorageKey(), state.activeEditorTab); } catch {}
  }
}
...
function setSession(session){
  ...
  if (els.loginHelp && loggedIn) els.loginHelp.textContent = "";
  if (loggedIn) {
...
function handleRequestAccess(){
  ...
  if (els.loginHelp) {
    els.loginHelp.textContent = "Solicite um novo acesso com a equipe responsável pelo painel administrativo.";
  }
}
...
function handlePreviewPublicPesquisa(){
  const slug = state.current?.slug || normalizeSlug(els.form?.querySelector('[name="slug"]')?.value || "");
  if (!slug) {
    adminMsg("Salve ou informe o slug da pesquisa antes de visualizar.", "err");
    return;
  }
  window.open(publicUrlFor(slug), "_blank", "noopener,noreferrer");
}
...
function getFilteredResearchRows(){
  const search = String(els.researchSearch?.value || "").trim().toLowerCase();
  const status = els.researchFilterStatus?.value || "all";
  return (researchCache || []).filter((pesquisa) => {
    const matchesSearch = !search || [pesquisa.titulo, pesquisa.slug, pesquisa.ano_base].some((v) => String(v || "").toLowerCase().includes(search));
    const matchesStatus = status === "all" || (status === "published" ? !!pesquisa.status : !pesquisa.status);
    return matchesSearch && matchesStatus;
  });
}

function renderFilteredResearchList(){
  renderResearchList(getFilteredResearchRows());
}
...
function renderResearchList(rows){
  ...
  if (els.listMsg) els.listMsg.textContent = rows.length ? "" : "Nenhum resultado para os filtros atuais.";
    ...
    return `
      <div class="r-item${activeClass}" data-id="${pesquisa.id}">
        ...
        <div class="r-actions">
          ...
          <button class="btn-mini" data-act="view" data-id="${pesquisa.id}">Visualizar</button>
...
async function loadResearchToForm(pesquisa){
  ...
  restoreLastTab(state.activeEditorTab || "config");
  loadPontos();
...
function fillForm(pesquisa, resumoKV = null){
  ...
  updatePdfPreview(pesquisa.leitura_url || pesquisa.relatorio_pdf_url || cfg.relatorioLeituraUrl || cfg.relatorioPdf || "");
  updateMapPreview(pesquisa.slug || "");
...
function getValue(name){
  const node = els.form?.querySelector(`[name="${name}"]`);
  return node?.value || "";
}
...
function setTab(tab, options = {}){
  ...
  const nextTab = tab || state.activeEditorTab || "config";
  ...
  const activeTab = Array.from(tabs).find((btn) => btn.dataset.tab === nextTab);
  const activePanel = Array.from(panels).find((panel) => panel.dataset.tabPanel === nextTab);
  ...
  if (persist && nextTab) {
    try { localStorage.setItem(getTabStorageKey(), nextTab); } catch {}
  }
...
function restoreLastTab(defaultTab = "config"){
  let stored = null;
  try { stored = localStorage.getItem(getTabStorageKey()); } catch {}
  const preferred = state.activeEditorTab || stored || defaultTab;
  const hasTab = preferred && document.querySelector(`#pesquisa-form .tabs .tab[data-tab="${preferred}"]`);
  setTab(hasTab ? preferred : defaultTab, { persist: false });
}
...
async function loadPontos(){
  ...
  state.pontos = data || [];
  renderPontosList();
  updateMapPreview(state.current?.slug || normalizeSlug(els.form?.querySelector("[name=slug]")?.value || ""));
}
...
document.addEventListener("click", async (event) => {
  ...
  try {
    ...
    if (action === "view") {
      const selected = researchCache.find((row) => String(row.id) === String(id));
      if (selected?.slug) {
        window.open(publicUrlFor(selected.slug), "_blank", "noopener,noreferrer");
      }
    }
